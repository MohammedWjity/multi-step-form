...
let stepsLocations = ["http://localhost:7700/mulity-step-form/index.html", "http://localhost:7700/mulity-step-form/step%202.html",
  "http://localhost:7700/mulity-step-form/step%203.html", "http://localhost:7700/mulity-step-form/step%204.html"]
...
if (window.localStorage.getItem("services") && window.localStorage.getItem("dataChoose") && window.localStorage.getItem("salary") && window.localStorage
  ...
  services.forEach(function(element) {
    if (window.localStorage.getItem("data-duration") === "Yearly") {
      element.serveiceSalary = `+$${element.serveiceSalary.slice(2,3)}0/yr`;
    }
    if (window.localStorage.getItem("data-duration") === "Monthly") {
      element.serveiceSalary = `+$${element.serveiceSalary.slice(2,3)}/mo`
    }
  ...
  if (window.localStorage.getItem("data-duration") === "Yearly") {
    totalSalary.innerHTML = `$${+card.querySelector("span").textContent.slice(1,card.querySelector("span").textContent.indexOf("/")) + total}/yr`;
  } else {
    totalSalary.innerHTML = `$${+card.querySelector("span").textContent.slice(1,card.querySelector("span").textContent.indexOf("/"))+total}/mo`;
  }
}
